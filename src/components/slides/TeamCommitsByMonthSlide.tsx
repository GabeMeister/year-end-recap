...
type TeamCommitsByMonthSlideProps = {
  part: string;
};

export default function TeamCommitsByMonthSlide({
  part,
}: TeamCommitsByMonthSlideProps) {
  const {
    data: commits,
    error,
    isLoading,
  } = useStats<TeamCommitsByMonth>({
    ...
    <div className="TeamCommitsByMonthSlide">
      {commits && (
        <>
          {part === "title" && (
            <div>
              <h1 className="text-5xl slide-fade-in">Commits by Month</h1>
            </div>
          )}
          {part === "main" && (
