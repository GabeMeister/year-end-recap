...
type TeamCommitsByWeekDaySlideProps = {
  part: string;
};

export default function TeamCommitsByWeekDaySlide({
  part,
}: TeamCommitsByWeekDaySlideProps) {
  const {
    data: commits,
    error,
    isLoading,
  } = useStats<TeamCommitsByWeekDay>({
  ...
  useEffect(() => {
    ...
    const x = d3
      .scaleBand()
      .domain(commits.map((c) => c.weekday))
    ...
    svg
      ...
      .data(commits)
      .join("rect")
      .attr("x", (d) => x(d.weekday) as number)
      .attr("y", (d) => {
        return y(0);
      })
      .attr("height", (_d) => {
    ...
    <div className="TeamCommitsByWeekDaySlide">
      {commits && (
        <>
          {part === "title" && (
            <div>
              <h1 className="text-5xl slide-fade-in">Commits by Week Day</h1>
            </div>
          )}
          {part === "main" && (
            <div className="overflow-y-scroll h-[700px] w-[910px]">
